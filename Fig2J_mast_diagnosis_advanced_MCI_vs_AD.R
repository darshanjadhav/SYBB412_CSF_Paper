...
output_dir <- "/home/mxg979/Project/SYBB412/results/Fig2J_MCI_AD/"
...
s@meta.data$Diagnosis<- rep("HControl", nrow(s@meta.data))
...
run_de <- function(cell_type) {
  ...
  # Write out results
  write.csv(degs, paste0(output_dir, cell_type_label, "_advancedonly_AD_vs_MCI_degs.csv"))
  
  # Create volcano plot
  volcano_plot(degs, title = paste0("AD vs MCI (Advanced only)\nin ", cell_type),
               file = paste0(output_dir, cell_type_label, "_advancedonly_AD_vs_MCI_volcano.pdf"),
